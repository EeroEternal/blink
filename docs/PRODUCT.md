...
- 可选的 V-Hub 传输通道（见 [VHUB.md](VHUB.md)）
...
| 组件 | 形态 | 典型用法 |
|------|------|----------|
| **`blink-sdk`** | Rust library | 自建 Rust 编排器、`cargo add blinkvm-sdk` |
| **`blink-server`** | 独立进程 + REST | XEnsemble 等控制面经 HTTP 调用（见 [XENSEMBLE.md](XENSEMBLE.md)） |
| **`blink-cli`** | 命令行 | 本地试用、调试、运维 |

核心能力在 **`blink-sdk`**；`blink-server` 与 `blink-cli` 都是对它的封装。**控制面（如 XEnsemble）走 `blink-server` HTTP，不必链接 Rust lib。**
...
| 执行面 API | `blink-server` REST（供 XEnsemble 等控制面调用，见 [XENSEMBLE.md](XENSEMBLE.md)） |
